...
const cartModalEl = document.querySelector(".cart_modal");
const btnCloseCart = document.querySelector(".cart_modal_close_btn");
const cartItemsEl = document.querySelector(".cart_items");
const cartItemsCountEl = document.querySelector(".cart_items_count");
const subtotalHolder = document.querySelector(".subtotal_holder");

export function renderLoading(loadingState) {
  const template = `
    <div class="loading_spinner flex justify-center col-span-full">
    ...
    const template = `
      ...
      product?.title?.length > 20 ? "..." : ""
          ...
          <div class="flex justify-between items-center gap-5 w-full">
            ...
            <button data-id="${
              product?.id
            }" class="btn add_to_cart_btn">Add to cart</button>
          </div>
        </div>
      </div>
    `;
      ...
      const template = `
        ...
        product.title
          ...
          <div class="w-full">
            ...
            <h5>$${product.price}</h5>
            <button class="remove_item_btn" data-id="${product.id}">
